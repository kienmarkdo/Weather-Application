...
const animateButton = (element) => {
    element.classList.toggle("none"); // hidden
...
export const updateDisplay = (weatherJson, locationObj) => {
    ...
    // sets background image after having determined what the weather is like
    ...
    // current conditions
    const ccArray = createCurrentConditionsDivs(weatherJson, locationObj.getUnit()); // ccArray == current conditions array
...
const fadeDisplay = () => {
    const cc = document.getElementById("currentForecast"); // current conditions/current forecast
    cc.classList.toggle("zero-vis");
    cc.classList.toggle("fade-in");

    const sixDay = document.getElementById("dailyForecast"); // daily forecast
...
const clearDisplay = () => {
    ...
    deleteContents(currentConditions); // delete everything within conditions div
...
const getWeatherClass = (icon) => {
    // icon is taken from Json. Icon has 3 characters. Separate the first two chars and the last char.
    ...
    // adjust display and background depending on the weather
    let weatherClass;
...
const createCurrentConditionsDivs = (weatherObj, unit) => {
    ...
    // create main image div with icon and description of current weather
...
const createMainImgDiv = (icon, altText) => {
    ...
    faIcon.ariaHidden = true; // for accessibility purposes
...
const translateIconToFontAwesome = (icon) => {
    // translates the first two characters that the OpenWeatherMap API gives us for their icons
